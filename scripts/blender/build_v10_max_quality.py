"""
Sponic Garden — v10 MAXIMUM QUALITY
4K UHD renders at 8192 samples, designed for multi-hour renders on Mac mini M4.
Enhancements over v9: 4K, 8192spp, caustics, volume scatter, subdivision surfaces,
more vegetation, interior elements, 8 cameras, dramatic sunset atmosphere.

Usage: blender --background --python build_v10_max_quality.py
"""
import bpy
import bmesh
import math
...
# ═══════════════════════════════════════════
print("=" * 60)
print("  SPONIC GARDEN v10 — MAXIMUM QUALITY BUILD")
print("=" * 60)
print("  Resolution: 3840x2160 (4K UHD)")
print("  Samples: 8192")
print("  Cameras: 8")
print("")
print("  WARNING: Each camera may take 30-90+ minutes to render")
print("  at these settings on a Mac mini M4.")
print("  Total estimated render time: 4-12 hours.")
print("=" * 60)
print("")
...
def pbr_corrugated(name, color1, color2, rough, metallic=0.5):
    ...
    # Strong bump from wave for corrugation ridges
    bump = nodes.new('ShaderNodeBump')
...
MAT = {
    ...
    # Lighting / tech — stronger string light emission (15.0 vs 12.0)
...
def make_greenhouse(name, cx, cy, w, d, h):
    ...
    for i in range(num):
        ...
        # Left/right rafters for A-frame peak
...
def make_organic_tree(name, cx, cy, coll=None, height=3.5, canopy_r=2.0):
    ...
    # v10: subdivisions=4 for more detailed tree canopies
...
def make_potted_plant(name, cx, cy, size=0.4, coll=None):
    """Terracotta pot with plant — scattered around buildings"""
...
def make_wall_vine(name, cx, cy, cz, width, height, face='S', coll=None):
    """Climbing vine on a wall — thin box with vine material"""
    coll = coll or C['plants']
    if face in ('S', 'N'):
...
def make_string_lights(name, x1, y1, x2, y2, height=3.5, num_bulbs=8, coll=None):
    """String lights between two points — emissive spheres with point lights"""
    ...
    # One point light per string for soft glow
...
def make_interior_area_light(name, cx, cy, cz, size_x=3, size_y=3, energy=5.0):
    """Warm interior area light visible through windows — 3000K warm white"""
    bpy.ops.object.light_add(type='AREA', location=(cx, cy, cz))
    light = bpy.context.active_object
    light.name = name
    light.data.energy = energy
    light.data.color = (1.0, 0.78, 0.50)  # ~3000K warm
...
bpy.ops.object.transform_apply(rotation=True)
cyl("Sauna_End", SX-4.4, SY+2, 1.2, 1.18, 0.08, MAT['cedar_dk'], C['spa'])
bpy.data.objects["Sauna_End"].rotation_euler[1] = math.radians(90)
cyl("Sauna_Chimney", SX-3, SY+2, 2.6, 0.08, 0.5, MAT['steel_frame'], C['spa'])

cp = box("CPlunge", SX+2, SY+2, 0.2, 3.0, 2.0, 1.2, MAT['concrete'], C['spa'], 0.02)
...
make_bench("SpaBench1", SX-0.5, SY-5.5, coll=C['spa'])
print("  Done")

# ═══════════════════════════════════════════
print("[5/14] Landscape...")

# Garden beds (NW) — v10: add small plant rows in soil
for row in range(4):
    for ci in range(6):
        x = -38 + ci*3.8
        y = 20 + row*3.5
...
# Flowers — v10: more flower clusters (50 instead of 30)
...
# v10: Extra flower clusters near buildings
extra_flower_spots = [
...
# ═══════════════════════════════════════════
print("[6/14] Potted plants + wall vines (doubled)...")

# Dense potted plants around building entrances — v10: doubled
pot_spots = [
    (1.5,-14), (-1.5,-14), (3,-14.5), (-3,-14.5),  # Welcome
    (-14.5,3), (-14.5,5), (-14.5,11),  # Dining
    (17.5,5), (17.5,8), (17.5,13),  # Education
    (-17.5,-3), (-17.5,-6),  # Maker
    (16.5,-3), (16.5,-6),  # Movement
    (7,-24), (17,-24),  # Spa
    (-32,-1.5), (-28,-1.5), (-32,-6.5), (-28,-6.5),  # Coffee
    # Around greenhouse
    ...
    # Central courtyard
    (3,3), (-3,3), (3,-3), (-3,-3),
    # v10: DOUBLED — additional potted plants
    (2.5,-14.5), (-2.5,-14.5), (0,-14.8),  # More at Welcome
    (-14.8,7), (-14.8,9),  # More at Dining
    (17.8,7), (17.8,11),  # More at Education
    (-17.8,-5), (-17.8,-8),  # More at Maker
    (16.8,-5), (16.8,-8),  # More at Movement
    (9,-24), (15,-24), (5,-26), (19,-26),  # More at Spa
    (-34,-3), (-26,-3), (-34,-5), (-26,-5),  # More at Coffee
    # More around greenhouse
...
# Climbing vines on building walls — v10: more vine coverage
make_wall_vine("Vine_Welcome_S", 3, -22.1, 2.5, 3, 3, 'S')
make_wall_vine("Vine_Welcome_E", 5.1, -18, 2.5, 4, 3, 'E')
make_wall_vine("Vine_Welcome_W", -5.1, -18, 2.5, 4, 3, 'W')  # v10
make_wall_vine("Vine_Dining_S", -20, 2.9, 2.5, 5, 3, 'S')
make_wall_vine("Vine_Dining_W", -29.6, 8, 2.5, 4, 3, 'W')
make_wall_vine("Vine_Dining_N", -20, 13.1, 2.5, 5, 3, 'N')  # v10
make_wall_vine("Vine_GH_S", -5, 17.4, 3.0, 6, 3.5, 'S')
make_wall_vine("Vine_GH_E", 10.1, 25, 3.0, 8, 3.5, 'E')
make_wall_vine("Vine_GH_W", -10.1, 25, 3.0, 8, 3.5, 'W')  # v10
make_wall_vine("Vine_GH_N", 5, 32.6, 3.0, 6, 3.5, 'N')  # v10
make_wall_vine("Vine_Maker_N", -22, -2.9, 3.0, 4, 3, 'N')
make_wall_vine("Vine_Maker_S", -22, -13.1, 3.0, 4, 3, 'S')  # v10
make_wall_vine("Vine_Spa_W", 6.9, -28, 2.0, 5, 2.5, 'W')
make_wall_vine("Vine_Spa_E", 17.1, -28, 2.0, 5, 2.5, 'E')  # v10
make_wall_vine("Vine_Edu_W", 16.9, 10, 2.5, 4, 3, 'W')  # v10
make_wall_vine("Vine_Edu_E", 27.1, 10, 2.5, 4, 3, 'E')  # v10
make_wall_vine("Vine_Move_S", 22, -13.1, 2.5, 4, 3, 'S')  # v10

print("  Done: doubled potted plants + more climbing vines")

# ═══════════════════════════════════════════
print("[7/14] Interior elements (visible through glass)...")

# Greenhouse interior — simple tables/shelves + plants
...
# Greenhouse shelving along walls
for si in range(3):
    sy = gh_cy - 5 + si * 5
...
# Movement studio interior — simple shelf/bench
...
print("  Done: greenhouse + movement studio interiors")
...
# ═══════════════════════════════════════════
print("[9/14] String lights (more runs)...")

# Between buildings — the warm atmosphere maker
# v10: original routes + additional runs
string_routes = [
    # Welcome to Dining/Maker
    (-4, -14, -15, 3, 4.0),
    (-4, -15, -17, -4, 4.0),
    # Welcome to Education/Movement
    (4, -14, 16, -5, 4.0),
    (4, -14, 17, 5, 4.0),
    # Around greenhouse
    (-10, 17, 10, 17, 4.5),
    ...
    (SX+8, SY-6, SX+8, SY+6, 3.0),
    # Central courtyard criss-cross
    (-5, -5, 5, 5, 4.0),
    (5, -5, -5, 5, 4.0),
    # Fire pit area
    (-28, -28, -20, -20, 3.0),
    (-20, -28, -28, -20, 3.0),
    # v10: Additional string light runs
    # Greenhouse lateral
    (-10, 25, 10, 25, 5.0),
    # Dining to coffee
    (-27, 3, -33, -1, 3.5),
...
# v10: Area lights inside buildings visible through windows (warm 3000K)
# Greenhouse — multiple interior lights
...
print("  Done: warm interior lights in all buildings")

# ═══════════════════════════════════════════
print("[13/14] Subdivision surfaces + render settings...")

# ═══════════════════════════════════════════
# ADD SUBDIVISION SURFACE TO ALL MESH OBJECTS
# ═══════════════════════════════════════════
subdiv_count = 0
...
# ═══════════════════════════════════════════
# RENDER SETTINGS — MAXIMUM QUALITY
# ═══════════════════════════════════════════
...
try:
    ...
    print("  WARNING: Falling back to CPU rendering (much slower)")

# Resolution: 4K UHD
scene.render.resolution_x = 3840
scene.render.resolution_y = 2160

# Samples: 8192 with tight adaptive threshold
...
try:
    ...
    try:
        ...
        print("  Color: AgX Medium High Contrast (Punchy not available)")
    except:
        try:
...
# 2. CAM_hero — same perspective but at 4K
...
# 5. CAM_greenhouse_detail — 85mm, f/2.0 DOF, close-up of greenhouse trusses + interior
...
# Point at greenhouse center (0, 25, 3)
...
# 6. CAM_firepit_evening — looking at fire pit with string lights visible
...
# Point at fire pit (-24, -24, 0.5)
dx, dy, dz = -24 - (-30), -24 - (-30), 0.5 - 3
cam6.rotation_euler = (math.radians(75), 0, math.radians(45))

# 7. CAM_coffee_bar — close-up of coffee bar with stools
...
# 8. CAM_spa_detail — close-up of barrel sauna + hot tubs
...
# RENDER
# ═══════════════════════════════════════════
RENDER_DIR = os.path.expanduser("~/Projects/sponic-garden-3d/renders/v10")
...
print("")
print("=" * 60)
print("  STARTING RENDERS — 8 cameras at 4K / 8192spp")
...
for ci, cam_name in enumerate(cam_names):
    ...
    out_path = os.path.join(RENDER_DIR, f"v10_{cam_name}.png")
...
save_path = os.path.expanduser("~/Projects/sponic-garden-3d/sponic-garden-v10.blend")
bpy.ops.wm.save_as_mainfile(filepath=save_path)

total_time = time.time() - build_start
print(f"\n{'=' * 60}")
print(f"  v10 COMPLETE — Total time: {total_time/60:.1f} minutes")
