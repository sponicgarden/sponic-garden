"""
Sponic Garden — v9 Concept Match
Target: match the illustrated concept art style from sponicgardens.com
Key changes: exposed trusses, string lights, dense vegetation, industrial materials,
sunset lighting, potted plants, climbing greenery, warmer atmosphere.

Usage: blender --background --python build_v9_concept.py
"""
import bpy
import bmesh
import math
...
MAT = {
    ...
    # Lighting / tech
    'string_bulb':   pbr("String_Bulb",  (1.0,0.88,0.55), 0.2, emission=12.0, emit_color=(1.0,0.88,0.55)),
...
# ═══════════════════════════════════════════
print("[6/12] Potted plants + wall vines...")

# Dense potted plants around building entrances
pot_spots = [
...
# Climbing vines on building walls
...
print("  Done: potted plants + climbing vines")
...
print("  Done: 16 string light runs")

# ═══════════════════════════════════════════
print("[9/12] Path lanterns...")
...
try: scene.view_settings.look = 'AgX - Medium High Contrast'
    ...
    try: scene.view_settings.look = 'AgX - Medium Contrast'
    except: pass
...
# SUNSET SKY — dramatic warm atmosphere
...
sky.sun_elevation = math.radians(12)   # low sunset
sky.sun_rotation = math.radians(-40)
try: sky.turbidity = 4.0  # hazy sunset
except: pass
...
# Key sun — sunset orange
bpy.ops.object.light_add(type='SUN', location=(30, -30, 20))
sun = bpy.context.active_object; sun.name = "Sun"
sun.data.energy = 6.0
sun.data.color = (1.0, 0.82, 0.55)  # warm sunset
sun.data.angle = math.radians(0.8)
sun.rotation_euler = (math.radians(12), math.radians(15), math.radians(-40))
...
print("  Done: Cycles 2048spp + sunset sky + warm lights")
...
bpy.ops.object.camera_add(location=(-16, 12, 5))
cam5 = bpy.context.active_object; cam5.name = "CAM_garden"
cam5.data.lens = 50; cam5.data.sensor_width = 36; cam5.data.clip_end = 500
cam5.data.dof.use_dof = True; cam5.data.dof.aperture_fstop = 2.8; cam5.data.dof.focus_distance = 6
cam5.rotation_euler = (math.radians(78), 0, math.radians(-115))
...
# RENDER
# ═══════════════════════════════════════════
RENDER_DIR = os.path.expanduser("~/Projects/sponic-garden-3d/renders/v9")
...
for cam_name in ['CAM_aerial', 'CAM_hero', 'CAM_pool_spa', 'CAM_entrance', 'CAM_garden']:
    ...
    out_path = os.path.join(RENDER_DIR, f"v9_{cam_name}.png")
    scene.render.filepath = out_path
    print(f"  Rendering {cam_name}...")
    bpy.ops.render.render(write_still=True)
    print(f"  Saved: {out_path}")

save_path = os.path.expanduser("~/Projects/sponic-garden-3d/sponic-garden-v9.blend")
